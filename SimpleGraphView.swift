...
struct SimpleGraphView: View {
    ...
    var body: some View {
        VStack(spacing: 12) {
            VStack(spacing: 120) {
                Text(NSLocalizedString("Trend", comment: "タイトル: トレンド"))
                    .font(.title)
                    .fontWeight(.bold)
                    .frame(maxWidth: .infinity, alignment: .center)
                
                VStack(spacing: 20) {
                    Text(NSLocalizedString("NumberOfHugs", comment: "グラフタイトル: スキンシップの回数"))
                        .font(.title2)
                        .fontWeight(.bold)
                        .frame(maxWidth: .infinity, alignment: .center)
            ...
            // 週切り替えボタン
            HStack {
                Button(NSLocalizedString("PreviousWeek", comment: "前の週")) {
                    currentWeekOffset -= 1
                    loadData()
                }
                ...
                Button(NSLocalizedString("NextWeek", comment: "次の週 →")) {
    ...
    func loadData() {
        ...
        data = (0..<7).map { offset in
            ...
            let count = allData.first(where: { item in
                let itemDate = calendar.startOfDay(for: item.date)
                return calendar.isDate(itemDate, inSameDayAs: normalizedDate)
            })?.count ?? 0
    ...
    func formattedMonthRange(from data: [DailyData]) -> String {
        ...
        formatter.setLocalizedDateFormatFromTemplate("yMMMM") // 年と月を表示
